...
if args.results_dir == '':
    args.results_dir = './caches/wopretrain/bt'+str(args.batch_size)+'/cache-' + datetime.now().strftime("%Y-%m-%d-%H-%M-%S-moco")
...
def collate_fn(batch):
    ...
    sorted_cap_lens, sorted_cap_indices = torch.sort(torch.tensor(cap_lens_tensor), 0, True)
    #print(image_input_tensor[0])
    return input_ids_tensor[sorted_cap_indices], \
...
def train(net, data_loader, train_optimizer, scaler, epoch, args):
    ...
    for input_ids, attention_mask, token_type_ids, cap_lens, image_input in train_bar:
        ...
        input_ids.to(device), attention_mask.to(device), token_type_ids.to(device), cap_lens.to(device), image_input.to(device)
...
def main():
    ...
    model = Gloria(L=args.loss_l,local_temp1=args.local_temp1, local_temp2=args.local_temp2, local_temp3=args.local_temp3).to(device)
    model.train()
    print(model.text_encoder)
