...
class IShuffledContrastiveModel(nn.Module):
    ...
    def shuffle_loss(self, input_ids_shuffled, attention_mask_shuffled, image_input):
        ...
        for txt in range(num_text):
            ...
            logits[:, txt] = (torch.sum(logits_text_per_image, dim=1) + torch.sum(logits_image_per_text, dim=1)) / 2
...
class TextShuffler:
    ...
    def shuffle_nouns_verbs_adj(self, ex):
        ...
        tokens = [token.text for token in doc]
        text = np.array(tokens)
        idx = [i for i, token in enumerate(doc) if token.tag_ in ['NN', 'NNS', 'NNP', 'NNPS', 'JJ', 'JJR', 'JJS', 'VB', 'VBD', 'VBG', 'VBN', 'VBP', 'VBZ']]
