...
parser.add_argument('--loss-t', default=0.1, type=float, help='shuffle loss temperature')
parser.add_argument('--shuffle-temp', default=0.07, type=float, help='Shuffle loss inside temperature')
parser.add_argument('--loss-l', default=0.1, type=float, help='local loss temperature')
parser.add_argument('--local-temp1', default=4.0, type=float, help='Local loss1 inside temperature')
parser.add_argument('--local-temp2', default=5.0, type=float, help='Local loss2 inside temperature')
parser.add_argument('--local-temp3', default=10.0, type=float, help='Local loss3 inside temperature')
...
if args.results_dir == '':
    args.results_dir = './new_caches_v7' + '/T'+str(args.loss_t) + '_L'+str(args.loss_l)+'_shuffle-temp'+str(args.shuffle_temp) +'/cache-' + datetime.now().strftime("%Y-%m-%d-%H-%M-%S-moco") 
    
isExist = os.path.exists('./new_caches_v7' + '/T'+str(args.loss_t) + '_L'+str(args.loss_l)+'_shuffle-temp'+str(args.shuffle_temp) )
if not isExist:
   # Create a new directory because it does not exist
   os.makedirs('./new_caches_v7' + '/T'+str(args.loss_t) + '_L'+str(args.loss_l)+'_shuffle-temp'+str(args.shuffle_temp) )
   print("The new directory "+ '\'./new_caches_v7' + '/T'+str(args.loss_t) + '_L'+str(args.loss_l)+'_shuffle-temp'+str(args.shuffle_temp) + "\' is created!")
print(args)
...
class ShuffledOpenIDataset(Dataset):
    def __init__(self, df, root_dir, transform=None, max_words=50):
        ...
        self.tokenizer= CXRBertTokenizer.from_pretrained("microsoft/BiomedVLP-CXR-BERT-specialized", revision="v1.1")
        ...
        perturb_functions = [shuffler.shuffle_nouns_and_adj, shuffler.shuffle_allbut_nouns_and_adj,
                             ...
                             shuffler.shuffle_all_words, shuffler.reverse_sentence, 
    ...
    def __getitem__(self, idx):
        ...
        for caption in caption_options:
            tokenizer_output = self.tokenizer(caption,
                                              return_tensors='pt',
                                              truncation=True,
                                              padding="max_length",
                                              max_length=50,)
        ...
        return input_ids_options, attention_mask_options, token_type_ids_options[0], cap_lens_options[0], img
...
def collate_fn(batch):
    ...
    input_ids_padded = [pad_sequence([seq for seq in option_set], batch_first=True) for option_set in zip(*input_ids_batch)]
    attention_mask_padded = [pad_sequence([seq for seq in option_set], batch_first=True) for option_set in zip(*attention_mask_batch)]

    # Convert lists of tensors to tensors
    token_type_ids_tensor = torch.stack(token_type_ids_batch)
    image_input_tensor = torch.stack(image_input_batch)

    sorted_cap_lens, sorted_cap_indices = torch.sort(torch.tensor(cap_lens_batch), 0, True)

    # Apply the sorting to the padded sequences for each option
    input_ids_tensor_sorted = [option[sorted_cap_indices] for option in input_ids_padded]
    attention_mask_tensor_sorted = [option[sorted_cap_indices] for option in attention_mask_padded]

    return input_ids_tensor_sorted, attention_mask_tensor_sorted, \
    token_type_ids_tensor[sorted_cap_indices], sorted_cap_lens, image_input_tensor[sorted_cap_indices]
...
train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=False, drop_last=True, collate_fn=collate_fn)
...
def train(net, data_loader, train_optimizer, scaler, epoch, args):
    ...
    for input_ids, attention_mask, token_type_ids, cap_lens, image_input in train_bar:
        ...
        token_type_ids.to(device), cap_lens.to(device), image_input.to(device)
...
def main():
    ...
    model = IShuffledContrastiveModel(T=args.loss_t, shuffle_temp=args.shuffle_temp, L=args.loss_l,\
    local_temp1=args.local_temp1, local_temp2=args.local_temp2, local_temp3=args.local_temp3).to(device)
    model.train()
    print(model)
